...
from pyoutlineapi.common_types import Validators
...
async def test_access_keys_and_server(access_key_dict, access_keys_list, server_dict):
    ...
    key = await client.create_access_key(name="Alice", port=12345)
    assert key.id == "key-1"

    key2 = await client.create_access_key_with_id("key-2", name="Bob")
    ...
    key_single = await client.get_access_key("key-1")
...
async def test_limits_and_metrics(server_metrics_dict, experimental_metrics_dict):
    ...
    status_json = await client.get_metrics_status(as_json=True)
    ...
    exp = await client.get_experimental_metrics("1h")
