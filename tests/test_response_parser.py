...
def test_parse_empty_dict_debug_log(caplog):
    with caplog.at_level(logging.DEBUG, logger="pyoutlineapi.response_parser"):
...
def test_parse_validation_error_many_details(caplog):
    ...
    class ManyErrorsModel(SimpleModel):
        ...
        def model_validate(cls, data):  # type: ignore[no-untyped-def]
            ...
            raise ValidationError.from_exception_data("ManyErrorsModel", errors)
...
def test_parse_unexpected_exception_logs_error(caplog):
    ...
    with caplog.at_level(logging.ERROR, logger="pyoutlineapi.response_parser"):
        with pytest.raises(OutlineValidationError):
            ResponseParser.parse({"id": 1, "name": "x"}, BadModel)
    assert any(
...
def test_extract_error_message_and_is_error_response():
    ...
    assert ResponseParser.extract_error_message(["bad"]) is None  # type: ignore[arg-type]
