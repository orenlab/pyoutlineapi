...
AddrInfo = tuple[object, object, object, object, SockAddr]
...
def test_mask_sensitive_data_list_branch():
    ...
    masked = mask_sensitive_data(data)
    assert masked["items"][0]["token"] == MASKED_VALUE
...
def test_build_config_overrides_and_json_serializable():
    overrides = build_config_overrides(timeout=10, enable_logging=True, foo=1)
...
def test_mask_sensitive_depth_limit():
    ...
    walker: object = masked
    found_error = False
    for _ in range(Constants.MAX_RECURSION_DEPTH + 3):
        if isinstance(walker, dict) and "_error" in walker:
            found_error = True
            break
        walker = walker.get("child", {}) if isinstance(walker, dict) else {}
